...
def printGraph(graph):
    ...
    nx.draw_networkx_edge_labels(graph, pos, edge_labels=nx.get_edge_attributes(
        graph, "weight"), font_size=12, rotate=False)
    ax = plt.gca()
    ax.margins(0.20)
    plt.axis("off")
    plt.show()
...
for user in users:
    if len(travellers) < maxUsersInCar:
        ...
        travelCost = float(lenKm * kmCost) / (len(travellers) + 1) + \
            calculateFixedCost(len(travellers) + 1, fixCost)
...
print("Travel cost for each user: ", totalCost / len(travellers) +
      calculateFixedCost(len(travellers), fixCost))
